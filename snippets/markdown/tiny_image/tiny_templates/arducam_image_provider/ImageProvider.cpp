...
bool GetImage(int image_width, int image_height, int channels, uint8_t* image_data) {
  ...
  bool capture_status = PerformCapture();
  if (capture_status != true) {
    return capture_status;
  }

  bool read_data_status = ReadData();
  if (read_data_status != true) {
...
bool GetDummyImage(int image_width, int image_height, int channels,
                                   uint8_t* image_data) {

  for(int y = 0; y < image_height; y++){
    for(int x = 0; x < image_width; x++){
      image_data[y * image_width + x] = x + y;
    }
  }
  return true;
}
